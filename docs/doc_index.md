...
- `docs/builtin-workflows/extract-experience.md`: 提取经验 → 项目无关最佳实践；用户明确要求时触发；落盘无 experience_target 则问，默认本仓 docs/best-practices/
- `docs/builtin-workflows/consolidate-memory.md`: 记忆巩固与遗忘（冲突检测 → 提纯合并 → 剪枝）；负面约束优先；用户明确要求时触发，不自动跑
...
- `docs/workflows/_TEMPLATE.md`: 业务 workflow 六段骨架；新建业务工作流时复制并改名
- `docs/workflows/choose-edit-style.md`: 开剪选型入口；手法表仅已实装项；同素材先扫 sources/cache；访谈走 FAQ 勿套峰会 plan
- `docs/faqs/访谈粗剪-分段异源与叙事选句.md`: 访谈跨场规则；分段异源/T0；枢纽与打戳；人耳排回音；升序 cut 契约；禁止 Whisper
- `docs/faqs/ElevenLabs-STT-401与禁止Whisper回退.md`: STT 401=换 key；禁止本地 Whisper 回退与假 cache
- `docs/workflows/cleanup-temp-media.md`: 交付初检后清 temp 可重建大媒体；dry-run → apply；`--theme` 须精确戳名；保留转写/sources/timeline
- `scripts/cleanup_temp_media.py`: cleanup-temp-media 入口（默认 dry-run；永不删 cache/转写/sources）
- `scripts/check_interview_timeline.py`: 访谈 timeline 交稿验收（时长 / 禁跨 T0 / `t<T0` 仅当时机位 / 半截启发式）；父与 subagent 交稿前跑
- `src/lapi/theme_dir.py`: temp 成片工作区抗碰撞命名（东八区 `YYYY_MM_DD_HH_MM`；allocate / resolve）
- `docs/workflows/summit-highlight.md`: 峰会高光手法总览；编排 dualcam + jinju；短名打戳 temp / output 不打戳；机位随片源；含仅补金句路径
- `docs/workflows/dualcam-auto-highlight.md`: 峰会手法叙事子步骤；plan 新建带戳 temp；cut 短名 resolve；CLI 仍双机缺口
- `docs/workflows/jinju-select.md`: 峰会手法金句子步骤（京剧=金句）；有几轴出几轴；推荐焊尾；无第四条成片；candidates/axes 仅 temp
